...
export function assertTelnyxTelephoneTargetNormalization() {
  const { frame } = buildTelnyxSessionUpdate({
    context: {
      ani: "tel:+15550000001",
      dnis: "tel:+15550000002",
    },
  });
  const variables = frame.session?.assistant?.dynamic_variables || {};
  const expected = {
    genesys_ani: "+15550000001",
    telnyx_end_user_target: "+15550000001",
    genesys_dnis: "+15550000002",
    telnyx_agent_target: "+15550000002",
  };
  for (const [name, value] of Object.entries(expected)) {
    if (variables[name] !== value) {
      throw new Error(
        `Audio Connector runtime invariant failed: ${name} did not remove the tel: URI scheme`
      );
    }
  }
  return true;
