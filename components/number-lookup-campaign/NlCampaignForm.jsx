...
import { Search, Users, RefreshCw, Play, AlertTriangle, Phone, User, FileText, Calendar, Link as LinkIcon, Hash, Home } from "lucide-react";
...
export default function NlCampaignForm({ accessToken }) {
  // Contact Lists state
  ...
  const contactListPrefix = process.env.NEXT_PUBLIC_NL_CONTACT_LISTS_PREFIX || "";
  ...
  const canStartCampaign = selectedContactList && contactListUri;
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            <Search className="size-6 text-telnyx-green" /> Number Lookup Campaign
          </CardTitle>
          <Link href="/">
        ...
        <CardContent className="space-y-6">
          ...
          {selectedContactList && (
            <div className="p-4 rounded-lg border border-telnyx-green/50 bg-telnyx-green/5">
              <div className="text-sm font-medium mb-2">Selected Contact List</div>
              <div className="text-sm">
                <span className="font-semibold">{selectedContactList.name}</span> - {selectedContactList.size} contacts
              </div>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Start Number Lookup Campaign
            </DialogTitle>
            <DialogDescription>
              Please confirm starting the <strong>{selectedContactList?.name}</strong> campaign. This
              will perform number lookups for all {selectedContactList?.size} contacts and update them
              with carrier information. This may incur significant charges.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
